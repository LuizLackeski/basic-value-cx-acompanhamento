...
*Atualizado em 2026-08-13 (Rodada 12). Itens marcados ✅ já estão implementados e publicados; os demais estão só registrados, aguardando implementação ou validação.*

## ⚠️ Ações pendentes do Luiz: Rodada 12 (migração de completude por deal_id) — publicação parcial, ver seção própria abaixo
A Rodada 12 migra `completude_instalacao_snapshot` de chave por `company_id` para chave por `deal_id` (resolve o achado do item 9 da Rodada 11 — completude hoje é por empresa, não por deal). **Só uma parte dos dados novos foi publicada nesta sessão** por causa do custo de transcrever manualmente ~5800 linhas de JSON pelo chat (ver detalhe na seção "Rodada 12" abaixo). Ação necessária do Luiz:
1. Baixar o zip `completude_instalacao_chunks_rodada12.zip` (enviado no chat) e seguir o `COMO_APLICAR.md` de dentro dele — é só copiar os 59 arquivos pra `data/chunks/` e fazer um `git push`, bem mais rápido do que eu publicar um por um.
2. Rodar `supabase/patch-completude-instalacao-por-deal.sql` no SQL Editor do Supabase (já publicado no repo, commit `924fd15`) — recria `completude_instalacao_snapshot` com `deal_id` como chave primária, adiciona `tickets_sync.deal_id`, ajusta `v_dashboard` e a policy de RLS.
...
## ✅/⚠️ Rodada 12 (2026-08-13) — completude por `deal_id` (não mais por empresa) + backfill de `tickets_sync.deal_id`
*Resolve o achado registrado no item 9 da Rodada 11: `completude_instalacao_snapshot` era calculado por `company_id`, então uma empresa com múltiplos deals (ex. venda antiga vencida + venda nova dentro do prazo) mostrava o mesmo status "dentro do prazo" pra todos os tickets, mascarando o deal individual vencido. Trabalho combinado no fim da Rodada 11 ("Roda agora já a atualização e ajustes que pedi"), executado nesta sessão.*

### O que foi feito
- **Query reescrita** (`queries/query-completude-instalacao-sync.sql`, commits `656efca`/`b8a4bf4`): o agregado de completude passou a ser por `deal_id` (`TICKET_ASSOCIATED_DEAL_ID`), não mais por `company_id` — cada deal agora carrega seu próprio `pct_completude`/`dentro_do_prazo`/`nota_instalacao`, sem "vazar" o status de uma venda nova pra uma venda antiga da mesma empresa.
- **Novo patch SQL** (`supabase/patch-completude-instalacao-por-deal.sql`, commit `924fd15`): recria `completude_instalacao_snapshot` com `deal_id text primary key` (era `company_id`), adiciona a coluna `tickets_sync.deal_id`, reconstrói o join de `v_dashboard` (`ci.deal_id = t.deal_id`) e a policy de RLS. **⚠️ Ainda não rodado pelo Luiz** — ver aviso no topo deste arquivo.
- **Backfill de `tickets_sync.deal_id`**: resolvido via HubSpot (`query_crm_data`, ticket → deal associado) — 1698 pares brutos, 1468 válidos (230 vieram "Unassigned", excluídos), 1279 `deal_id` distintos (várias empresas têm mais de um ticket por deal). Publicado em `data/chunks/ticket_deal_ids-00/01/02.json` (commits `04e172b`/`3272099`/`de3b576`) e **conferido byte-a-byte** — completo.
- **Dataset de completude por deal**: query completa rodada no Databricks (5810 linhas, resposta em chunk único, sem paginação). Achadas e resolvidas 3 duplicatas de `deal_id` (mesmo deal associado a duas datas de assinatura diferentes rio acima, no Databricks) — dedup determinístico mantendo a linha com maior `dias_restantes_prazo` (estado mais atual). Resultado final: **5807 deals únicos**, ordenados por `deal_id`, divididos em 59 arquivos de 100 linhas (`completude_instalacao-00.json` a `-58.json`).
- **`scripts/upsert_to_supabase.py` atualizado** (commits do 3º subagent + `c0fe3c51e7c17c95cf04c77947509cf1a14d8b5c`, verificado byte-a-byte): nova função `update_ticket_deal_ids()` (PATCH-only, nunca cria linha) alimentando `tickets_sync.deal_id`; upsert de `completude_instalacao_snapshot` trocado de `on_conflict="company_id"` para `on_conflict="deal_id"`; e um novo `_run_soft(label, fn, *args)` envolvendo essas duas chamadas em `main()` — se o patch SQL acima ainda não tiver rodado (colunas/PK não existem ainda), a etapa é **pulada com aviso no log**, sem abortar o resto do sync (tickets, ESN, team_members etc. continuam rodando normalmente). Isso era um risco real: sem essa proteção, qualquer sync rodado antes do patch SQL manual quebraria o script inteiro no meio.

### ⚠️ Incidente durante a publicação dos chunks de completude (corrigido, mas deixou publicação incompleta)
- Duas tentativas de despachar sub-agentes em paralelo pra publicar os 12 arquivos `completude_instalacao-*.json` (500 linhas/arquivo, esquema antigo) bateram no limite semanal de uso da assinatura no meio da tarefa, deixando o GitHub num estado parcialmente corrompido: `-00.json` truncado em 90000 bytes (JSON inválido), `-06.json` truncado em 30000 bytes, e vários outros arquivos sem tocar.
- Diagnosticado (sem confiar no relato dos sub-agentes — reconferido direto via `get_file_contents`) e corrigido publicando (eu mesmo, sem sub-agente) os arquivos `-00`, `-01` e `-02` do esquema novo (100 linhas/arquivo, por `deal_id`), cada um verificado byte-a-byte (tamanho batendo exato com o arquivo local) depois do push.
- **Publicação interrompida aqui por custo**: cada arquivo de ~27KB precisa passar inteiro pela minha própria saída de texto pra eu conseguir chamar a ferramenta do GitHub (não há acesso direto à API do GitHub por `curl`/`gh` nesta sandbox — só as ferramentas MCP). Publicar as 56 arquivos restantes (`-03` a `-58`) um por um dessa forma ficaria proibitivamente caro. **Solução**: os 59 arquivos corretos foram embalados num zip (`completude_instalacao_chunks_rodada12.zip`) e enviados ao Luiz no chat, com um `COMO_APLICAR.md` explicando como copiar pra `data/chunks/` e dar um `git push` — muito mais rápido e confiável feito direto do computador dele.
- **Estado exato no GitHub ao final desta sessão**: `completude_instalacao-00.json`, `-01.json` e `-02.json` já estão no esquema novo (por `deal_id`) e conferidos. `-03.json` a `-10.json` ainda têm o esquema antigo (por `company_id`, de antes da Rodada 12) ou dados truncados. `-11.json` a `-58.json` ainda não existem no repositório. **Ninguém deve considerar a migração de completude completa até o zip ser aplicado.**

### Verificação feita antes de publicar
- Contagens do backfill de `ticket_deal_ids` conferidas (1698 brutos → 1468 válidos, sem duplicata de `ticket_id`, sem campo vazio).
- Dataset de completude por deal conferido contra a resposta bruta do Databricks (5810 linhas → 5807 após dedup determinístico, documentado em comentário no script de geração).
- Cada arquivo publicado nesta sessão (`completude_instalacao-00/01/02.json`, `ticket_deal_ids-00/01/02.json`, `scripts/upsert_to_supabase.py` nas duas versões) conferido byte-a-byte (`get_file_contents` + comparação de tamanho/conteúdo) imediatamente depois do push — inclusive o achado e a correção do incidente de corrupção dos sub-agentes.
